...
public:
   ...
   TVector &normalize();
   static TVector &normalize(const TVector &v, TVector &result) { result = v; return result.normalize(); }
   static TVector normalize(const TVector &v) { return TVector(v).normalize(); }
   ...
   // Magnitude
   double length() const { return (isValid() ? (isUnit() ? 1.0 : sqrt(pow(X(), 2) + pow(Y(), 2) + pow(Z(), 2))) : 0.0); }
   double lengthSqr() const { return (isValid() ? (isUnit() ? 1.0 : pow(X(), 2) + pow(Y(), 2) + pow(Z(), 2)) : 0.0); }
   ...
   double dist(const TVector &v) const { return (*this-v).length(); }
   double distSqr(const TVector &v) const { return (*this-v).lengthSqr(); }
